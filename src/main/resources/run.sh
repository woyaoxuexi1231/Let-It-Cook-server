...
IMAGE_NAME="${IMAGE_NAME:-let-it-cook:latest}"
CONTAINER_NAME="${CONTAINER_NAME:-let-it-cook}"
HOST_PORT="${HOST_PORT:-15842}"
CONTAINER_PORT="${CONTAINER_PORT:-15842}"
...
resolve_jar_file() {
  ...
  if [[ ${#jars[@]} -eq 0 ]]; then
    ...
    echo "        JAR_FILE=let-it-cook.jar bash run.sh"
    exit 1
  fi
